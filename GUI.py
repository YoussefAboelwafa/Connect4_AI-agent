...
class ConnectFour:
    ...
    def resize_game_window(self, width, height,player):
        ...
        self.screen = pygame.display.set_mode((COLUMN_COUNT * SQUARE_SIZE, ROW_COUNT * SQUARE_SIZE), pygame.RESIZABLE)
        self.animation_frames = [pygame.transform.scale(frame, (COLUMN_COUNT * SQUARE_SIZE, ROW_COUNT * SQUARE_SIZE))
    ...
    def draw_board(self, current_player):
        ...
        for i in range(COLUMN_COUNT,COLUMN_COUNT+3):
            for j in range (ROW_COUNT+1):
                pygame.draw.rect(self.screen, LIGHT_GRAY, (i * SQUARE_SIZE, j * SQUARE_SIZE, SQUARE_SIZE, SQUARE_SIZE))
        ...
        if not self.player_vs_player:
            ...
            solve_tree_button = pygame.Rect((COLUMN_COUNT ) * SQUARE_SIZE+50, 4 * SQUARE_SIZE, 2 * SQUARE_SIZE, SQUARE_SIZE)
            font = pygame.font.Font(None, 30)
            text = font.render("Solve Tree", True, BLACK)
            self.screen.blit(text, (solve_tree_button.x + solve_tree_button.width // 2 - text.get_width() // 2, solve_tree_button.y + solve_tree_button.height // 2 - text.get_height() // 2))
                    ...
                    sys.exit()
                        ...
                        if backIconRect.collidepoint(event.pos):
                            ...
                            for child in root1.children:
                                if first_child_x+child_offset*child_index<=event.pos[0]<=first_child_x+child_offset*child_index+100 and first_child_y<=event.pos[1]<=first_child_y+100:
    ...
    def draw_tree(self, node, x, y ):
        ...
        for child in node.children:
            pygame.draw.line(self.screen, (0, 0, 0), (x + 180 // 2, y + 150), (child_x + 100 // 2, child_y), 2)
    ...
    def draw_mini_board(self,state,size,x,y):
        ...
        for c in range(COLUMN_COUNT):
            for r in range(ROW_COUNT):
                ...
                if piece == 1:
                    pygame.draw.circle(self.screen, RED, (col + MINISQUARESIZE // 2, row
                                                            + MINISQUARESIZE // 2), MINI_PIECE_RADIUS)
                elif piece == 2:
                    pygame.draw.circle(self.screen, YELLOW, (col + MINISQUARESIZE // 2, row
                                                                + MINISQUARESIZE // 2), MINI_PIECE_RADIUS)
                elif piece == 0:
                    pygame.draw.circle(
                        self.screen, WHITE,
    ...
    def ai_menu(self):
        ...
        while ai_menu:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if  300 <= event.pos[1] <= 350:
                        ...
                        selected_ai_engine = "pruning"  # Adjust for your actual AI engine
                        ai_menu = False
            ...
            option_text = font.render("Minimax 'with pruning' ", True, BLACK)  # Adjust for your actual AI engine
            ...
            difficulty_text = font.render(f"Difficulty: {ai_difficulty}", True, DARK_BLUE)
    ...
    def show_winner_popup(self,player):
        ...
        if player==-1:
            ...
            win_text = font.render(f"It's a tie!", True, GREEN if player == 1 else GREEN)
    ...
    def draw_properties(self,turn):
        ...
        if not self.player_vs_player:
            difficulty_text = font.render(f"AI DIFFICULTY {self.difficulty}",True,GREEN)
            self.screen.blit(difficulty_text, ((COLUMN_COUNT)*SQUARE_SIZE, 3 * SQUARE_SIZE))
        pygame.display.update()
    def tie_move(self):
        for i in range (7):
